...
class test_GesRPCMethods(unittest.TestCase):
    ...
    def setUp(self):
        ...
        dirone = os.path.abspath(os.path.join(self.base_path,'repos/repoone'))
        os.makedirs(dirone)
        if subprocess.call('git init --quiet --bare "%s"' % dirone):
            raise Exception('Initing sample git repo failed.')
        os.makedirs(
            os.path.abspath(os.path.join(self.base_path,'repos/folderone/'))
            )
